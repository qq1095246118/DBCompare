...
Tag 归属为准，政治、地缘政治、经济、金融、科技和加密六个大类默认分别选择
最多 20 条记录，默认最终最多 120 条分类记录。每个大类依次按以下指标补足
配置的每类限额：
...
`--page-limit` 只控制 Gamma API 每页请求数量，允许范围是 `1–20`。不要传
`100`；Gamma 返回的嵌套市场对象较大，大分页会造成不必要的资源压力。
